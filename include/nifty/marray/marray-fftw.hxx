#pragma once
#ifndef NIFTY_MARRAY_MARRAY_FFTW_HXX
#define NIFTY_MARRAY_MARRAY_FFTW_HXX
...
public:
    ...
    FFT(const nifty::marray::Marray<double>&, nifty::marray::Marray<std::complex<double> >&);
    ~FFT();
    void execute();
...
public:
    ...
    IFFT(const nifty::marray::Marray<std::complex<double> >& in, nifty::marray::Marray<double>& out);
    ~IFFT();
    void execute();
...
inline
FFT<S>::FFT(
    const nifty::marray::Marray<double>& in,
    nifty::marray::Marray<std::complex<double> >& out
) {
    if(in.coordinateOrder() != nifty::marray::CoordinateOrder::FirstMajorOrder) {
        throw std::runtime_error("Marray not in first-major order");
    }
    if(out.coordinateOrder() != nifty::marray::CoordinateOrder::FirstMajorOrder) {
        throw std::runtime_error("Marray not in first-major order");
    }
    if(in.dimension() != 2) {
...
template<class S>
IFFT<S>::IFFT(
    const nifty::marray::Marray<std::complex<double> >& in,
    nifty::marray::Marray<double>& out
) {
    if(in.coordinateOrder() != nifty::marray::CoordinateOrder::FirstMajorOrder) {
        throw std::runtime_error("Marray not in first-major order");
    }
...
} // namespace nifty::marray
} // namespace nifty

#endif // #ifndef NIFTY_MARRAY_MARRAY_FFTW_HXX
