...
namespace graph {
    ...
    public:
        ...
        // constructor for sub-volume
        RagCoordinates(const RagType & rag, const std::vector<int64_t> & subBegin, const std::vector<int64_t> & subEnd, const int nThreads = -1)
            : rag_(rag), storage_(rag) {
            initStorage(subBegin, subEnd, nThreads);
        }
    ...
    private:
        ...
        void initStorage(const std::vector<int64_t> & subBegin, const std::vector<int64_t> & subEnd, const int nThreads);
    ...
    template<size_t DIM, class RAG_TYPE>
    template<class T>
    inline void RagCoordinates<DIM, RAG_TYPE>::writeBothCoordinates(const int64_t edgeId, const T edgeVal, marray::View<T> & out) const {
    ...
    template<size_t DIM, class RAG_TYPE>
    template<class T>
    inline void RagCoordinates<DIM, RAG_TYPE>::writeLowerCoordinates(const int64_t edgeId, const T edgeVal, marray::View<T> & out) const {
    ...
    template<size_t DIM, class RAG_TYPE>
    template<class T>
    inline void RagCoordinates<DIM, RAG_TYPE>::writeUpperCoordinates(const int64_t edgeId, const T edgeVal, marray::View<T> & out) const {
    ...
    void RagCoordinates<DIM, RAG_TYPE>::initStorage(const int nThreads) {
        ...
        const auto & labelsProxy = rag_.labelsProxy().labels();
    ...
    template<size_t DIM, class RAG_TYPE>
    void RagCoordinates<DIM, RAG_TYPE>::initStorage(const std::vector<int64_t> & subBegin, const std::vector<int64_t> & subEnd, const int nThreads) {

        typedef std::vector<std::vector<int32_t>> CoordinateVectorType;

        const auto numEdges = rag_.numberOfEdges();
        const auto & labelsProxy = rag_.labelsProxy().labels();
        Coord begin, end, shape;
        for(int d = 0; d < DIM; ++d) {
            begin[d] = subBegin[d];
            end[d] = subEnd[d];
        ...
        marray::Marray<uint32_t> labels(shape.begin(), shape.end());
        tools::readSubarray(labelsProxy, begin, end, labels);
