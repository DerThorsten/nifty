...
//namespace detail {
    ...
    // we give the adjacency graph (= normal graph for multicut / lifted graph for lifted multicut) as template
    ...
    public:
        ...
        TwoCut(
            ...
            const GraphType & adjacencyGraph,
            const Settings & settings = Settings()
        )
        :   objective_(objective),
            settings_(settings),
            graph_(objective.graph()),
            adjacencyGraph_(adjacencyGraph),
        ...
        double optimizeTwoCut(
            ...
            auto computeDifferences = [&](const SET & varSet, const uint64_t labelAA, const uint64_t labelBB){
                for(const auto var : varSet){
                    ...
                    for(auto adj : adjacencyGraph_.adjacency(var)){
            ...
            for (auto k = 0; k < settings_.numberOfIterations; ++k){
                ...
                for(const auto adj : adjacencyGraph_.adjacency(m.v)){
    ...
    private:
        ...
        const GraphType & adjacencyGraph_;
