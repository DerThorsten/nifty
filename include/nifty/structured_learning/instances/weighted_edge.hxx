...
namespace instances{
    ...
    public:
         ...
         * 
         */
        template<class WEIGHT_VECTOR, class GRADIENT_VECTOR, class BINARY_OPERATOR>
        void accumulateGradient(
            const WEIGHT_VECTOR & weights,
            GRADIENT_VECTOR & gradient,
            BINARY_OPERATOR && binaryOperator
        )const{
            for(const auto & indexFeaturePair : indexFeaturePairs_){
                const auto wIndex = weights[indexFeaturePair.first];
