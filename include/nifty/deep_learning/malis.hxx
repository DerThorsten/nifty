...
template<unsigned DIM, typename DATA_TYPE, typename LABEL_TYPE, typename GRADIENT_TYPE>
double malisLossAndGradient(
        const marray::View<DATA_TYPE> & affinities,
        const marray::View<LABEL_TYPE> & groundtruth,
        marray::View<GRADIENT_TYPE> & gradients,
        const float beta
    ...
    NIFTY_CHECK_OP(gradients.shape(DIM),==,DIM,"Number of gradient channels must match !");
    // check that shapes match
    for(int d = 0; d < DIM; ++d) {
        NIFTY_CHECK_OP(affinities.shape(d),==,groundtruth.shape(d),"Affinity shape does not match gt shape!");
        NIFTY_CHECK_OP(affinities.shape(d),==,gradients.shape(d),"Affinity shape does not match gradients shape!");
    }
            ...
            for (itU = overlaps[setU].begin(); itU != overlaps[setU].end(); ++itU) {
                for (itV = overlaps[setV].begin(); itV != overlaps[setV].end(); ++itV) {
                    ...
                    if (itU->first == itV->first) {
                        gradients(affCoord.asStdArray()) += static_cast<GRADIENT_TYPE>(nPair)*static_cast<GRADIENT_TYPE>(beta);
                        //debug output
                    ...
                    // we add nPairs if we join two nodes in different gt segments
                        ...
                        gradients(affCoord.asStdArray()) -= static_cast<GRADIENT_TYPE>(nPair)*static_cast<GRADIENT_TYPE>(1.0f - beta);
                        //debug output
                        //std::cout << "Gt-labels: " << itU->first << " , " << itV->first << std::endl;
