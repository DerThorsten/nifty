...
class Erosion2D(Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(shape=k_shape,
                                      initializer='uniform',
                                      trainable=True)
        super(Erosion2D, self).build(input_shape)  # Be sure to call this somewhere!
    ...
    def call(self, x):
        ...
        return K.tf.nn.erosion2d(x, self.kernel, strides=strides, rates=rates, padding=padding, name=self.name)
...
class Conv1x1Erosion2D(Layer):
    def __init__(self, filters, kernel_size, padding='same', strides=None, name=None, dilation_rate=None,**kwargs):
        super(Conv1x1Erosion2D, self).__init__(**kwargs)
        self.filters = filters
        self.erosion_layer = Erosion2D(filters=filters, kernel_size=kernel_size,
